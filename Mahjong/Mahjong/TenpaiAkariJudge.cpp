...
void taj::free_yaku_table(yakuTable* first)
{
...
void taj::start()
{
	...
	yakuman = false;
	current = new yakuTable;
	MemoryLeakMonitor::addMonitor(current, sizeof(yakuTable), "YAKU_TABLE CURRENT");
	current->basicpt = 0;
	current->huutotal = 0;
	current->yakutotal = 0;
	current->tail = NULL;
...
int taj::tenpai_detect_internal(judgeRequest* rpai, judgeResult* resultEx)
{
	...
	akari_status = rpai->akariStatus, norelease = rpai->norelease;
...
int taj::tenpai_detect(const judgeRequest* rpai, judgeResult* resultEx)
{
	...
	rpai2.akariStatus = rpai->akariStatus;
